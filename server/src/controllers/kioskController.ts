...
export const getDashboardData = (req: Request, res: Response): void => {
  try {
    ...
    const family = db.prepare<{ name: string; profile_image: string | null; holiday_mode: number; slideshow_mode: string; slideshow_interval: number }>(
      'SELECT name, profile_image, holiday_mode, slideshow_mode, slideshow_interval FROM families WHERE id = ?'
    ).get(familyId);
